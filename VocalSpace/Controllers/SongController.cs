...
namespace VocalSpace.Controllers
{
    public class SongController : Controller
    {
        ...
        /// AJAX 歌曲按讚邏輯
        /// </summary>
        [HttpPost("/Song/AddToPlayHistory/{songId}")]
        public async Task<IActionResult> AddToPlayHistory(long songId)
        {
            // 取得使用者ID
            long? userId = HttpContext.Session.GetInt32("UserId");

            var playHistory = await _context.PlayHistories
                .FirstOrDefaultAsync(ph => ph.UserId == userId && ph.SongId == songId);

            if (playHistory != null)
            {
                playHistory.PlayTime = DateTime.Now;
